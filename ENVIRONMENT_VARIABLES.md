...
| Variable                | Required | Default                 | Description                                   |
|-------------------------|----------|-------------------------|-----------------------------------------------|
| `NEXT_PUBLIC_API_URL`   | Yes      | `http://localhost:80`   | API URL via Nginx (accessible from browser)   |
...
- `NEXT_PUBLIC_API_URL` should point to the Nginx reverse proxy (`http://localhost:80` or `http://localhost`), which routes requests to the appropriate microservice.

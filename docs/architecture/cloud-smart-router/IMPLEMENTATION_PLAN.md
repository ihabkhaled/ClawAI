...
| #     | Batch                                                                                                                                                                                        | Workspaces                  | Unblocks                                            | Status                                                                                                         |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| **0** | **Privacy filter gates the invocation** (§0). Pure safety fix + tests. No schema change.                                                                                                     | routing                     | Everything — no cloud adapter may exist before this | ✅ Done                                                                                                        |
| 1     | Provider enum + `ModelDeployment` + `CapabilityEvidence` (additive migration)                                                                                                                | routing                     | 3, 8                                                | ✅ Done                                                                                                        |
| 2     | `SeedExecution` + seed machinery + `prisma.config.ts` + `prisma/seed.js`                                                                                                                     | routing                     | 4                                                   | ✅ Done                                                                                                        |
| 3     | `RouterInferenceCoordinator` + provider interface + Gemini/Ollama-Cloud/legacy adapters + canonical error taxonomy + per-entry breaker + strict output + 1 bounded repair + attempt recorder | routing                     | 5, 6                                                | ✅ Done                                                                                                        |
| 4     | `RouterConfiguration` + `RouterChainEntry` + revisions + `cloud-smart-router-default-v1` seed                                                                                                | routing                     | 7, 9                                                | ✅ Done                                                                                                        |
| 5     | Per-request immutable snapshot + eligibility filter + `CloudRouterManager` promoted to hot path, v1 as rollback (see D11)                                                                    | routing                     | 6                                                   | ✅ Done                                                                                                        |
| 6     | `RouterProviderAttempt` + `RoutingCandidateScore` persistence                                                                                                                                | routing                     | 7                                                   | ✅ Done                                                                                                        |
| 7     | Typed trace events: shared-types union → routing publishes → chat carries on Runtime V2                                                                                                      | shared-types, routing, chat | 8                                                   | ✅ Done                                                                                                        |
| 8     | Frontend: discriminated event union, `eventId` dedupe, sequence guard, Auto timeline, i18n ×13                                                                                               | frontend                    | —                                                   | ✅ Done                                                                                                        |
| 9     | Admin Smart Router page: API + 6 tabs + RBAC + chain reorder + i18n ×13                                                                                                                      | routing, frontend           | —                                                   | ✅ Done                                                                                                        |
| 10    | Discovery / enrichment / lifecycle validation                                                                                                                                                | connector, routing          | 11                                                  | ✅ Done                                                                                                        |
| 11    | Learning V4–V7 (V6 gated on the tenant-scoping migration, D8)                                                                                                                                | routing                     | —                                                   | ⚠️ V4/V5 done, V6 partial by design, V7 correctly not started — see §9                                         |
| 12    | Routing lab: 300-case corpus, fault injection, replay, evidence manifest                                                                                                                     | routing                     | release                                             | ✅ Done (harness + first-pass evidence; full 1,000/100/100/100 programme still open — see §9 verification row) |
...
## 9. Batch 11 (Learning Evolution) — final status

All 12 numbered batches plus Batch 11's four V-tiers are now built, gated, and pushed. Batch 11's
own status, stated per the pack's own V4–V7 spec
(`07_LEARNING_EVOLUTION_V4_V7_PROMPT.md`) rather than glossed over:

| Tier                                      | Status                                    | What shipped                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **V4** Shadow/replay/evaluation           | **Done**                                  | `RouterShadowEvaluationManager` — legacy-vs-cloud shadow comparison, evaluator/rubric versioning, redacted (preview-only) storage.                                                                                                                                                                                                                                             |
| **V5** Learned scores                     | **Done**                                  | ADR-069: `RouterModelProfile`/`RouterTopicProfile` confirmed as the one production learning system; `RouterLearnedScore` retired dead-but-harmless. Confidence intervals, outlier controls, evaluator attribution, versioned batch recalibration + rollback added.                                                                                                             |
| **V6** Tenant/domain/user personalization | **Partial, by explicit design (ADR-070)** | Workspace tier only (`global → domain/task → workspace`) — `RouterWorkspacePrior`, fully built and tested, but **inert**: no current caller populates a workspace id, so it changes no live behavior today. User tier deferred — routing-service has no `userId` concept, and threading one in is cross-service (chat-service) work outside this batch's `routing`-only scope. |
| **V7** Bounded contextual bandit          | **Correctly not started**                 | The pack requires V7 "only after V1–V6." V6's user tier and live activation are still open, so V7 has not been attempted — this is the sequencing rule working as designed, not an omission.                                                                                                                                                                                   |

**What "done" means here, concretely:** every V4/V5/V6 code path is real, wired into the live
`calibrateDecision`/`ingestExecutionOutcome` hot path, and covered by tests that exercise it —
not scaffolding with no caller. V6's inertness is a stated scope boundary (no upstream caller
sends a workspace id yet), not a hidden gap: reactivating it is "chat-service starts passing one,"
no further routing-service design work.

**Concrete follow-ups this leaves, in dependency order:**

1. Thread a real workspace id from chat-service into the RabbitMQ payloads that become
   `RoutingContext`/`RoutingCompletedEventPayload` — its own reviewed, cross-service change.
2. Add a `userId` concept to routing-service and extend `RouterWorkspacePrior`'s hierarchy to the
   user tier the pack specifies.
3. Only then, V7 — champion/challenger bandit over low-risk traffic, capped exploration, kill
   switch, auditable policy version, per the pack's own gate list.

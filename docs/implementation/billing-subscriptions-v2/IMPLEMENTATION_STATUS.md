...
1. _Medium_ — `Plan.priceMonthly/priceYearly` are `Decimal(12,2)`, duplicating the
...
2. _Medium_ — `POST /admin/billing/refunds` accepts a client-supplied `amountMinor`.
   Admin-gated and permission-checked, but a money value crossing the trust boundary.
3. _Low_ — `Plan.allow*` booleans and `PlanFeatureRule` both express feature access.

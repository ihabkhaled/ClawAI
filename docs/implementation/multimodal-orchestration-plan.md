...
- Cancellation (§72): chunked upload has no abort; an image asset stored just
  before a winning cancel can be left unreferenced; Stop before the read-aloud
  POST answers does not cancel.
...
| ---------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Read aloud       | `POST /chat-messages/:id/speech/cancel` | GENERATING → CANCELLED; stored segments stay; in-flight holds RELEASED; late answers discarded; POST during wind-down 409 `TTS_CANCEL_PENDING`, then resumes |
...
| Chunked upload   | —                                       | **Gap:** no abort on client or server                                                                                                                        |

...
_Implemented, unit-gated; live QA not run._ Owner-only, idempotent, 200 with the
state after the call; semantics in [ADR-120 addendum 3](../13-adr/adr-120-clawai-owns-multimodal-orchestration.md#addendum-3--user-cancellation-pack-72-2026-09-25),
rule 37 item 20, rule 42 item 21.

| Surface          | Route                                    | Result                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Read aloud       | `POST /chat-messages/:id/speech/cancel`  | GENERATING → CANCELLED; stored segments stay; in-flight holds RELEASED; late answers discarded; POST during wind-down 409 `TTS_CANCEL_PENDING`, then resumes. A Stop pressed before `POST …/speech` answered is remembered (one flag) and sent once when the answer is GENERATING                                                                                        |
| Image generation | `POST /images/:id/cancel`                | QUEUED/STARTING → CANCELLED, no call, no hold; GENERATING → result discarded, hold RELEASED, no successor; ComfyUI targeted interrupt; retry → successor                                                                                                                                                                                                                 |
| Video processing | `POST /files/:id/processing/cancel`      | FAILED `PROCESSING_CANCELLED`; ffmpeg killed; transcription hold RELEASED; chat reason `video_processing_cancelled`, never native                                                                                                                                                                                                                                        |
| Recording        | (client)                                 | Existing `cancel` releases every track — already tested                                                                                                                                                                                                                                                                                                                  |
| Chunked upload   | `DELETE /files/upload/chunked/:uploadId` | Owner-only, idempotent, 200 `{uploadId, aborted}` (missing / expired / someone else's → `aborted: false`, never an error); temp chunk dir removed now, not at TTL. Client: removing a tile still uploading aborts its `AbortController` — no further chunk, no retry/backoff, one DELETE (the TTL sweep is the backstop); unmounting a composer aborts what is in flight |

No migration: `ImageGenerationStatus.CANCELLED` already existed. New shared enum
member: `VideoProcessingFailureReason.PROCESSING_CANCELLED` (shared-types rebuild).
Deploy: shared-types → file-service → image-service → chat-service → frontend (a
frontend Stop against an old chat-service 404s harmlessly; it is logged, not shown).
...
- ~~On phones the floating feedback rail (− / +) and "Jump to latest" cover
  assistant text (`08-matrix-mobile-390x844-portrait.png`); at 740×360 the − sits
  on the action rail.~~ Fixed 2026-09-25 in CSS: while the launcher is expanded,
  below `sm` each transcript row, and from `sm` to `md` the thread body row,
  give up `--floating-rail-gutter` on the inline-end side (`:has()`, no JS);
  "Jump to latest" is an icon-only circle below `sm` inside that strip. Pinned
  by `chat-surface-layout-contract.test.ts`; the device matrix is not re-run.
...
| Media metrics (§67)         | Per-service `/api/v1/metrics` (chat, file, image) from `MetricsRegistry` (`@claw/shared-utilities`, no library), labels bounded to declared values (`other` otherwise): transcription attempts/job duration, video outcome/duration, media queue wait, attachment delivery mode, vision-helper outcome, TTS segments/first audio/job status, image generations/duration. Scrape jobs (chat by DNS, per replica), dashboard `claw-multimodal.json`, ADR-113 addendum, skill `add-a-service-metric.md`. Not verified against a running Prometheus.                                                                     |
| Read-aloud voice picker     | `User.ttsVoice` (auth migration `20260926100000_add_user_tts_voice`), validated against `TTS_VOICES_BY_PROVIDER` (30 Gemini prebuilt voices, verified in the docs; 6 OpenAI tts-1 voices); chat reads it once per POST, the voice is part of the replay key, a voice of the other provider falls back to that provider's default; settings card (shadcn Select, 13 locales). Live QA not run.                                                                                                                                                                                                                        |

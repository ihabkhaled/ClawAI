...
| Variable                           | Required     | Default          | Description                                                                                                                                                                                                                                                                                            |
| ---------------------------------- | ------------ | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `RESEARCH_HEADLESS_RENDER_ENABLED` | No           | `true`           | Hard kill switch for the Patchright headless tier of the fetch escalation chain (ADR-121)                                                                                                                                                                                                              |
| `CLAW_SCRAPER_PROFILES`            | No           | (empty)          | Which scraping sidecars exist: subset of `crawl4ai,flaresolverr,firecrawl` (compose profiles)                                                                                                                                                                                                          |
| `FIRECRAWL_POSTGRES_PASSWORD`      | No           | internal default | Firecrawl sidecar's private Postgres; generated by the installers                                                                                                                                                                                                                                      |
| `FIRECRAWL_BULL_AUTH_KEY`          | No           | (empty)          | Firecrawl sidecar's queue-admin key; generated by the installers                                                                                                                                                                                                                                       |
| `CRAWL4AI_API_TOKEN`               | For Crawl4AI | (empty)          | Secret. Bearer token shared by the `crawl4ai` container and research-service. Crawl4AI 0.9.x binds `0.0.0.0` and requires it only when set; blank = loopback-only sidecar and the CRAWL4AI tier is skipped as unavailable. Generated (and preserved) by the installers; by hand `openssl rand -hex 32` |

Which tiers of the escalation chain run is DB-level (`FetchStrategyConfig`,
admin `PATCH /api/v1/research/fetch-strategies/:kind`), not env. The env
switch above can only take the in-process browser OUT (a resource lever, not
a safety one: every in-page request is SSRF-checked either way).
`CLAW_SCRAPER_PROFILES` is env only because which containers exist must be
decided before anything runs; a started sidecar is still off until enabled
in the DB. See ADR-121 and `skills/add-a-fetch-strategy.md`.

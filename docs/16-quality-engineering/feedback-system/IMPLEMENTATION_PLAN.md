...
| Screenshot capture on mobile          | **Not offered at all** — the button is hidden unless `navigator.mediaDevices.getDisplayMedia` exists                                                 | No mobile browser implements screen capture for web pages: neither iOS Safari nor Android Chrome exposes `getDisplayMedia`. The original build showed the button everywhere, so every tap on a phone produced "Capture unavailable" — and, because `feedback.screenshot.failed` had no dictionary entry, it printed the raw key. Upload stays available and on a phone opens the photo library, where the OS screenshot already is. |

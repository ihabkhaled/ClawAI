...
| A completed answer costs at most one page-1 refetch                    | **Still measured at 1, occasionally 2.** One confirmed race (the poll interval staying armed one render past `streamCompletedAt`) is fixed and unit-tested. A live duplicate persists with a timing signature (1.4s apart) inconsistent with that race — leading unconfirmed hypothesis is a React StrictMode dev-only double-invocation, untested against a production build (see chat-pipeline-audit-2026-09.md, D6). Independently, the poll now targets page 1 only regardless of how much history is loaded (B9, 2026-09-11), so whatever the occasional duplicate's true cause turns out to be, its cost is capped at one bounded page fetch rather than scaling with pages scrolled back through. | `use-thread-detail.test.tsx` guards the fixed race; `use-virtualized-messages.test.tsx` guards the page-1-only poll. The remaining live symptom has no guard because its cause is not yet confirmed. |   🔴   |

...
### B8 — Offset pagination over a head-growing list — **FIXED 2026-09-11**
...
> **Root-caused and closed, not mitigated.** The first pass (same day) removed
> the primary _trigger_ — B9's fix stopped the poll from refetching page 2+
> during a run — and added id-based dedup as a symptom guard. That left the
> underlying mechanism itself untouched: a manual `fetchPreviousPage` could
> still race a concurrently-arriving message and skip a row entirely, which no
> amount of client-side dedup can recover, because the row was never fetched.
>
> The actual fix is what the mechanism required: `GET
/chat-messages/thread/:threadId` now takes a cursor (`before`, a message id)
> instead of a page number. `ChatMessagesRepository.findByThreadId` uses
> Prisma's native `cursor: { id: before }, skip: 1` instead of a numeric
> `skip` — "everything before this exact row" is a fixed meaning regardless of
> what gets inserted above it, so there is no window left to shift. The
> response's `meta.nextBefore` (the oldest returned message's id, or `null`
> once a page comes back short) replaces `page`/`totalPages` end to end:
> backend DTO, repository, service, controller; frontend type, repository
> method, `useVirtualizedMessages`'s `getNextPageParam`, both cache-merge
> utilities, and the nine orchestration poll hooks that fetch "the newest
> messages" (now `before: undefined` instead of a hardcoded `page: 1`). The
> `qa-lab` test client's page-walking loop was rewritten to walk the cursor
> instead. Full accounting in
> [rules/06 §13](../../rules/06-frontend-queries-and-cache.md).
>
> The id-based dedup added on the first pass stays as defense-in-depth, not as
> the fix — a duplicate id reaching the render list is no longer supposed to
> be reachable by construction, but the check costs nothing to keep. B6's
> normalized message store remains a separate, larger, explicitly out-of-scope
> decision — this fix did not require it, because a cursor-paginated list
> endpoint and a normalized client-side entity store are independent concerns.
...
**Sections A, B, C, D and E are all closed; one named, out-of-scope
architectural decision remains.** D3 closed 2026-09-11: connection health is
...
closed 2026-09-11 — B8's fix (cursor pagination) turned out not to need B6's
normalized store, which remains the one deliberately out-of-scope
architectural decision: a client-side cache-shape rewrite touching every read
site in `hooks/chat/`, orthogonal to how the server paginates. Outside all of
it: long-chat performance, tracing, accessibility, the contract suite,

...
| **GPU detection (host)**      | `scripts/claw.sh` cross-platform probe: NVIDIA (`nvidia-smi -L`), AMD ROCm (`/dev/kfd`), Intel/Vulkan (`/dev/dri/render*`), macOS warn. Picks the matching `docker-compose.{dev,prod}.gpu-{nvidia,rocm,vulkan}.yml` overlay automatically.                                                                                                                                                                            |
...
| **Compose**                   | Legacy `docker-compose.dev.yml` + `docker-compose.yml` deleted; `claw.sh up` is THE entrypoint. Per-vendor GPU overlays added (6 files: dev/prod × nvidia/rocm/vulkan). Service is in `docker-compose.dev.services.yml` + `docker-compose.prod.services.yml`; DB is in `docker-compose.dev.databases.yml` + prod mirror.                                                                                              |

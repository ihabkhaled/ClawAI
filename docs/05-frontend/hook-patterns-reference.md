...
| `useMessageComposer`            | chat       | The composer's ONE controller hook: composes translation, plan features and composer state, and resolves the control variant from `MEDIA_QUERY_LG_UP`. Height is not its business — `RichPromptTextarea` owns autosize (ADR-088).                                                                                                                    |
| `useModelPicker`                | chat       | The shared ModelPicker's ONE controller hook. Seeds cmdk's HIGHLIGHT (`Command value`, which is not the selection) from the current choice when the picker opens, so a 173-model list scrolls to it instead of opening at the top; the highlight then belongs to the keyboard. Also resolves sheet-vs-popover from `MEDIA_QUERY_BELOW_MD` (ADR-090). |

...
| Audit Conversational Context             | `audit-conversational-context.md`              | Someone reports the AI forgetting a conversation; regression-gating the context composer against a live deployment                                                                      |
| Debug an Attachment a Model Cannot Read  | `debug-an-attachment-the-model-cannot-read.md` | A model replies that it cannot read an attached PDF, image or document. Start from the file row, not the model.                                                                         |
| Add an Archive Format                    | `add-an-archive-format.md`                     | Expand a new archive format or accept a new archive MIME label: signature, MIME maps, chat-service mirror, the tests the 7-Zip (WASM) path needs, and the traps (ADR-114).              |
| Change a Transactional Email             | `change-a-transactional-email.md`              | Reword, add or translate an email the auth service sends, or debug one that arrived in the wrong language or looked broken in a real client.                                            |
| Verify a Batch Live (API + Playwright)   | `verify-a-batch-live.md`                       | Before calling any batch done: exercise it against the running stack, clear the service worker first, and record the observed outputs.                                                  |
| Add a Localized Money Surface            | `add-a-localized-money-surface.md`             | Show an amount in the visitor's currency: the two calls that do it, the difference between a price and a charge, and the shortcuts that quietly break it.                               |
| Add a Fetch Strategy                     | `add-a-fetch-strategy.md`                      | Register a new web-fetch escalation strategy (TLS-impersonating client, reader proxy, archive fallback) into research-service's pluggable fetch-strategy layer (ADR-121).               |
...
| Keep an Agent Honest                     | `keep-an-agent-honest.md`                      | Applying AI-Psychiatry discipline (locked DoD, evidence not assumption, no hidden recursion, one control at a time) when the plugin isn't installed                                     |
...
| Change the Product Positioning           | `change-the-product-positioning.md`            | The slogan, description, pillars or flagship list changes: the one canonical home, the code audit behind every claim, and the fan-out to README, wiki, routers and bootstrap (ADR-126). |
| Verify a Responsive Layout in a Browser  | `verify-responsive-layout-in-browser.md`       | A layout, header, shell or growing input changed: the size matrix, the zoom row, the content states, and the evidence a redesign has to produce.                                        |
| Measure the Chat Network Baseline        | `measure-chat-network-baseline.md`             | Before/after numbers for any change claiming less traffic, polling, payload or telemetry: two thread states, decoded vs transfer size, printed cadences.                                |
...
| Add an OpenAI-compatible Provider        | `add-an-openai-compatible-provider.md`         | A new provider whose API is OpenAI-shaped (Groq, OpenRouter, Cerebras, etc.) — one `CONNECTOR_PRESETS` entry, no new adapter class (ADR-117).                                           |
...
| Add Pagination to a List                 | `add-pagination-to-a-list.md`                  | A table can grow past one screen, or someone is "pressing Next forever". One shared control; do not hand-roll a ninth.                                                                  |
| Give a Surface the Same Context as Chat  | `give-a-surface-the-same-context-as-chat.md`   | You are adding or fixing anything that asks a model a question and it needs history, files, memory, cross-thread context or research.                                                   |
| Debug a Video the Model Cannot Read      | `debug-a-video-the-model-cannot-read.md`       | A video sits in "processing", has no transcript, or the model answers as if nothing was attached. ffmpeg, probe, plan limit, transcription reason, frames.                              |
| Verify Multimodal Routing Live           | `verify-multimodal-routing-live.md`            | Prove image/voice/video/TTS/image-generation routing on a running stack: `qa/test-multimodal.sh` (free vs paid) + the live per-model capability matrix; deploy order and NOT-RUN list.  |
| Add a Voice Note or Transcription Path   | `add-a-voice-note-or-transcription-path.md`    | Audio is uploaded and something must read it, or a connector gains speech-to-text.                                                                                                      |
| Add an Outbound HTTP Destination         | `add-an-outbound-http-destination.md`          | A call fails with "refusing a host this service does not call", or a service must reach a host it has never reached before.                                                             |
...
| Run the Coding-Agent Release Rounds      | `run-coding-agent-release-rounds.md`           | After every coding-agent release: repair the baked shared-package drift, find the tool-capable connector models, run the live rounds, and triage what fails (rule 53)                   |
...
| Watch Production Health                  | `watch-production-health.md`                   | Is it up, what went down, for how long: the /observability status section, its admin API, and how to change it without leaking a host or adding a fan-out (ADR-113, plan B3)            |
| Add a Service Metric                     | `add-a-service-metric.md`                      | A counter/histogram a health check cannot see: bounded labels (enum values, `other`), `MetricsRegistry`, `/api/v1/metrics` internal only, scrape job + test (ADR-113 addendum)          |
| Add a Grafana Dashboard                  | `add-a-grafana-dashboard.md`                   | Dashboards are code under `infra/grafana/dashboards/`; the UI cannot save one; stable uid, correct datasource uid, recreate not restart, try-it-live sandbox (ADR-115)                  |
| Apply Akinator Without the Plugin        | `apply-akinator-without-the-plugin.md`         | Any repo-changing task when the `akinator` plugin isn't installed: the 12 stations in order and the ClawAI path each artifact lands in (rule 55)                                        |

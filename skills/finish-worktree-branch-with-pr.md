...
## Before the first commit in a new worktree

**Confirm the hooks actually exist.** `core.hooksPath` is `.husky/_`, which is
generated by the `prepare` script — and `npm install --ignore-scripts`, the
usual way to set a worktree up quickly, skips `prepare`. Git then finds no hook
files and runs nothing, **silently**: no error, no warning, exit 0. Every commit
lands ungated without anyone typing `--no-verify`.

```bash
ls .husky/_ >/dev/null 2>&1 || npx husky
```

Caught on `feat/attachment-understanding` (2026-09-12) after four commits had
already landed with no lint-staged, no prettier on markdown, and no automatic
`.ai/` regeneration. The tell is downstream, not at commit time:
`npm run knowledge:verify` reports stale generated files the hook should have
staged, and `prettier --check` fails on files it should have formatted. If a
commit produced no lint-staged output, the hook did not run.

...
5. **Reason about overlap, not about sides.** `left`/`right` tests need a second
...
6. **The sheet primitive has physical sides only.** `SheetContent` accepts
...
7. **`.safe-bottom` and `.safe-top` assign padding, so they beat `p-*`.** They
...
8. **Cap any computed clearance.** A tall floating panel must not be able to push

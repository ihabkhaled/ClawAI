...
8. **Error payloads never carry a cost ceiling, a margin, or a provider rate.** A
   402 carries `availableMicroUsd` and `requiredMicroUsd` and nothing else. The
...
| Rule                                   | Mechanism                                                                                                                                                                                                                                                  |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 — surface enum member                | **Unit test** — `payg-credit-surfaces.spec.ts` asserts one metered path per `PaygSurface` member. A new member with no caller, or a caller with no member, fails it.                                                                                       |
...
| 9 — classification in auth only        | **Architecture test** — `shared-entitlements` must not import provider lists or rate types. **Review checklist** for calling services.                                                                                                                     |

...
| 7 — append-only ledger                 | **Unit test** — asserts `(grant + purchased) - reserved = SUM(ledger)` after a concurrency run. NOT `wallet = SUM(ledger)`: a RESERVATION row is negative while the money is still in the buckets, so the gross comparison reports a false drift equal to the outstanding holds. **Repository review**: no `update`/`delete` on the ledger model.                                                                                                                                                                                                  |

...
const PACKS = [
  {
    ...
    keywords: ['auth', 'login', 'logout', 'jwt', 'token', 'refresh', 'session', 'password', 'argon2', 'rbac', 'permission'],
    ...
    pitfalls: ['Refresh-token rotation must invalidate the old token', 'Never log tokens/passwords', 'Ownership checks live in the service, not the controller'],
  },
  {
    pack: 'rabbitmq-event',
    keywords: ['event', 'rabbitmq', 'publish', 'consume', 'subscribe', 'queue', 'dlq', 'exchange', 'message.'],
    ...
    pitfalls: ['Add the pattern to packages/shared-types EventPattern first', 'Every producer needs a documented consumer', 'Handlers must not swallow errors silently'],
  },
  {
    pack: 'database-migration',
    keywords: ['prisma', 'schema', 'migration', 'model', 'column', 'table', 'mongoose', 'backfill', 'database'],
    summary: 'Prisma/Mongoose schema or migration change',
    reviewers: ['database-reviewer', 'migration-reviewer'],
    validation: ['cd apps/<service> && npx prisma migrate dev --name <name>', 'npm run affected:test'],
    pitfalls: ['Migrations must be additive/reversible', 'Each service owns its DB — no cross-DB queries', 'Repository files never throw'],
  },
  {
    pack: 'chat-streaming',
    ...
    pitfalls: ['SSE routes need @SkipLogging + proxy_buffering off in nginx', 'Store an error ASSISTANT message on failure or polling spins forever', 'Never pass JWT in URL query params'],
  },
  {
    pack: 'ai-provider-connector',
    keywords: ['connector', 'provider', 'openai', 'anthropic', 'gemini', 'bedrock', 'deepseek', 'grok', 'adapter', 'model sync'],
    ...
    pitfalls: ['Wrap the vendor SDK in an adapter — no raw SDK imports in services', 'API keys are AES-256-GCM encrypted at rest', 'Never expose encryptedConfig in responses'],
  },
  {
    pack: 'model-routing',
    ...
    pitfalls: ['Privacy keywords must force local routing', 'ROUTER-role models never appear in the chat selector'],
  },
  {
    pack: 'frontend-feature',
    keywords: ['component', 'page', 'hook', 'ui', 'react', 'tsx', 'button', 'form', 'dialog', 'modal', 'frontend', 'tailwind', 'shadcn'],
    ...
    pitfalls: ['TSX = render only; no hooks/consts/inline types/sub-components', 'All text via t() in all 9 locales', 'Use shadcn/ui — no raw select/input/textarea'],
  },
  {
    pack: 'workspace-connector',
    keywords: ['workspace', 'github', 'gitlab', 'jira', 'slack', 'oauth', 'webhook', 'sync', 'confluence', 'drive'],
    ...
    pitfalls: ['OAuth secrets never leave the backend', 'Sync jobs must be idempotent + rate-limited'],
  },
  {
    pack: 'infrastructure',
    keywords: ['docker', 'compose', 'nginx', 'env', 'port', 'volume', 'gpu', 'ci', 'workflow', 'vercel'],
    ...
    pitfalls: ['New service/DB → ALL split compose files in the same commit', 'New env var → .env.example + installers + docs', 'New service → nginx + health + shared-constants'],
  },
  {
    pack: 'documentation',
    ...
    pitfalls: ['i18n.types.ts + locales are one atomic change', 'Generated .ai files are never hand-edited'],
  },
];
...
const DEFAULT = {
  ...
  validation: ['npm run affected:list', 'cd apps/<service> && npm run typecheck && npm run lint && npm test'],
  pitfalls: ['Controller → Service → Repository layering', 'No inline types/enums/consts in logic files', 'Every new function needs a test'],
};

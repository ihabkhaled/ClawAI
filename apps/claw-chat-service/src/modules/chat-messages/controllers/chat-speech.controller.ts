 ...
 * poll; POST `…/speech/cancel` is the owner's Stop (200, idempotent). None
 * ever waits on a provider.
 */
...
export class ChatSpeechController {
  ...
  @Post(':id/speech/cancel')
  @HttpCode(HttpStatus.OK)
  async cancel(
    @Param(new ZodValidationPipe(messageSpeechParamsSchema)) params: MessageSpeechParamsDto,
    @CurrentUser() user: AuthenticatedUser,
  ): Promise<MessageSpeechStateResponse> {
    return this.speech.cancel(user.id, params.id);
  }

...
import { CHAIN_ORPHAN_RUN_MAX_AGE_MS } from '../../constants/chain-cadence.constants';
import { ChainRepository } from '../../repositories/chain.repository';
import { ChainOrphanRunRecoveryManager } from '../chain-orphan-run-recovery.manager';
...
describe('ChainOrphanRunRecoveryManager', () => {
  let manager: ChainOrphanRunRecoveryManager;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ChainRepository, useValue: { markOrphanedRunsAsFailed } },
      ],
    }).compile();

    manager = module.get(ChainOrphanRunRecoveryManager);
  });
  ...
  it('sweeps chain runs older than CHAIN_ORPHAN_RUN_MAX_AGE_MS', async () => {
    ...
    expect(markOrphanedRunsAsFailed).toHaveBeenCalledWith(
      new Date(now.getTime() - CHAIN_ORPHAN_RUN_MAX_AGE_MS),
      expect.stringContaining('Orphaned chain run recovered'),
    );
  });
  ...
  it('is a no-op when the scheduler is disabled', async () => {
    ...
    const disabledManager = module.get(ChainOrphanRunRecoveryManager);
  ...
  it('logs sweep result on startup when runs are recovered', async () => {
    markOrphanedRunsAsFailed.mockResolvedValue(3);
    await manager.onModuleInit();
    expect(markOrphanedRunsAsFailed).toHaveBeenCalled();
  });

  it('handles an empty sweep cleanly', async () => {

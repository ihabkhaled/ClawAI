...
export class OutlookCalendarAdapter implements WorkspaceAdapter {
  ...
  async syncObjects(accessToken: string, _deltaToken?: string): Promise<SyncResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Outlook Calendar sync HTTP ${String(response.status)}: ${text.slice(0, 200)}`);
    }
  ...
  async validateOAuthAppConfig(appCredentials: AdapterAppCredentials): Promise<HealthCheckResult> {
    ...
    return probeOAuthAppCredentials({
      ...
      interpret: (payload, status) => {
        ...
        if (error === 'invalid_grant' || /AADSTS70008|AADSTS54005|AADSTS9002313/.test(description)) {
  ...
  getDefaultScopes(): string[] {
    return ['Calendars.Read', 'offline_access', 'User.Read'];
  }
  ...
  async fetchObjectDetails(
    ...
    const response = await fetch(`${MICROSOFT_GRAPH_API_BASE}/me/events/${encodeURIComponent(externalId)}`, {
      headers: { Authorization: `Bearer ${accessToken}`, Accept: 'application/json' },
      signal: AbortSignal.timeout(HEALTH_CHECK_TIMEOUT_MS),
    });
    ...
    return {
      ...
      externalCreatedAt: event.createdDateTime !== undefined ? new Date(event.createdDateTime) : null,
      externalUpdatedAt: event.lastModifiedDateTime !== undefined ? new Date(event.lastModifiedDateTime) : null,
  ...
  private mapEventToSynced(event: OutlookCalendarEvent): SyncedObject {
    return {
      ...
      metadata: this.buildMetadata(event),
      externalCreatedAt: event.createdDateTime !== undefined ? new Date(event.createdDateTime) : undefined,
      externalUpdatedAt:

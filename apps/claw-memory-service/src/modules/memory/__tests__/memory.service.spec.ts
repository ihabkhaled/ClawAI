...
const mockMemoryRepository = (): Record<keyof MemoryRepository, jest.Mock> => ({
  ...
  findLearnedPreferences: jest.fn(),
...
describe('MemoryService', () => {
  ...
  beforeEach(() => {
    ...
    const extractionManager = { extract: jest.fn().mockResolvedValue(void 0) };
  ...
  describe('getMemory', () => {
    ...
    it('should throw BusinessException when user does not own memory', async () => {
      ...
      await expect(service.getMemory('mem-1', 'other-user')).rejects.toThrow(BusinessException);
    });
  });
  ...
  describe('updateMemory', () => {
    ...
    it('should throw BusinessException when user does not own memory', async () => {
      ...
      await expect(service.updateMemory('mem-1', 'other-user', { content: 'New' })).rejects.toThrow(
        BusinessException,
      );
    });
  });
  ...
  describe('deleteMemory', () => {
    ...
    it('should throw BusinessException when user does not own memory', async () => {
      ...
      await expect(service.deleteMemory('mem-1', 'other-user')).rejects.toThrow(BusinessException);
    });
  });

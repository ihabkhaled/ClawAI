...
export default tseslint.config(
  ...
  },
  {
    ...
    rules: {
      'no-restricted-syntax': ['error', ...logicFileRestrictions, { selector: 'TryStatement', message: 'Controllers must not use try/catch.' }, { selector: 'ThrowStatement', message: 'Controllers must not throw.' }],
    },
  },
  {
    files: ['src/**/*.repository.ts'],
    rules: {
      'no-restricted-syntax': ['error', ...logicFileRestrictions, { selector: 'ThrowStatement', message: 'Repositories must not throw business exceptions.' }],
    },
  },
  {
    files: ['src/**/*.module.ts', 'src/**/*.guard.ts', 'src/**/*.interceptor.ts', 'src/**/*.filter.ts', 'src/**/*.pipe.ts', 'src/**/*.adapter.ts'],
    rules: {
      'no-restricted-syntax': ['error', ...logicFileRestrictions],
    },
  },

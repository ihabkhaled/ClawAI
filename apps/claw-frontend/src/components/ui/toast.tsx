...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-0 z-[100] flex max-h-screen w-full flex-col gap-2 p-4 safe-top sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col-reverse md:max-w-[420px]",
      className,
    )}
    {...props}
  />
...
function ToastProgressBar({ variant, durationMs }: ToastProgressBarProps): React.ReactElement {
  ...
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none absolute inset-x-0 bottom-0 h-1 overflow-hidden rounded-b-md bg-foreground/5"
    >
      <div

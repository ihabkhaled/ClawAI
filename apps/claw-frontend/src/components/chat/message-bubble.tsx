...
function MessageBubbleBase({
  ...
  const imageGenerationId = typeof metadata?.['generationId'] === 'string' ? metadata['generationId'] : undefined;
  const isFileGeneration = metadata?.['type'] === 'file_generation';
  const fileGenerationId =
    typeof metadata?.['generationId'] === 'string' && isFileGeneration ? metadata['generationId'] : undefined;
  ...
  const workflowReason = typeof metadata?.['workflowReason'] === 'string' ? metadata['workflowReason'] : null;
    ...
    <div className={cn('group/bubble flex w-full', isUser ? 'justify-end' : 'justify-start')}>
      ...
      >
        ...
        {!isUser && isTruncatedAtContextLimit ? (
          <div role="alert" className="flex w-full items-start gap-2 rounded-md border border-warning/40 bg-warning-surface px-3 py-2 text-xs text-warning">
        ...
        <div
          className={cn(
            ...
            isUser ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground group-hover/bubble:bg-card/80',
          )}
        >
          {isUser ? (
            <>
          ...
          {!isUser && isImageGeneration && imageGenerationId ? (
            <ImageGenerationBubble generationId={imageGenerationId} prompt={message.content} isAutoMode={message.routingMode === RoutingMode.AUTO} />
          ) : null}
        ...
        {isUser && message.content.trim().length > 0 ? (
          <div className="flex items-center gap-1 self-end transition-opacity md:opacity-0 md:group-hover/bubble:opacity-100 md:focus-within:opacity-100">
            <CopyButton text={message.content} size={ComponentSize.SM} label={t('chat.copyMessage')} className="h-7 w-7 text-muted-foreground" />
          </div>
        ) : null}
          ...
          <div className="flex max-w-full flex-wrap items-center gap-1.5">
            ...
            {memoryCount > 0 ? (
              <Badge variant="secondary" className="gap-1 text-xs">
                ...
                {memoryCount === 1 ? t('chat.oneMemory', { count: memoryCount }) : t('chat.manyMemories', { count: memoryCount })}
              </Badge>
            ) : null}
            {contextFileIds.length > 0 ? (
              <Badge variant="secondary" className="gap-1 text-xs" aria-label={`${t('nav.files')}: ${contextFileIds.length}`}>
        ...
        {!isUser ? (
          <div className="flex flex-wrap items-center gap-1 transition-opacity md:opacity-0 md:group-hover/bubble:opacity-100 md:focus-within:opacity-100">
            ...
            {onRegenerate ? (
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground" onClick={() => onRegenerate(message.id)} aria-label={t('chat.regenerate')}>
            ...
            {onFeedback ? (
              <>
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn('h-7 w-7 p-0', message.feedback === MessageFeedback.POSITIVE ? 'text-primary' : 'text-muted-foreground')}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn('h-7 w-7 p-0', message.feedback === MessageFeedback.NEGATIVE ? 'text-destructive' : 'text-muted-foreground')}

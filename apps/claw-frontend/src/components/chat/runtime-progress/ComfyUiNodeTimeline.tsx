...
export function ComfyUINodeTimeline({
  ...
  return (
    <section
      className={cn(
        'border-border bg-card/40 flex flex-col gap-1.5 rounded-xl border p-3',
        className,
      )}
      aria-label={t('runtimeProgress.comfyui.timeline')}
    >
      <div className="touch:text-xs text-muted-foreground mb-1 text-[11px] font-semibold tracking-[0.18em] uppercase">
      ...
      {entries.map((entry, idx) => {
        ...
        if (entry.cached) {
          ...
          icon = <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-sky-500" aria-hidden />;
        } else {
          icon = (
            <CircleDashed className="text-muted-foreground h-3.5 w-3.5 shrink-0" aria-hidden />
          );
        }
        ...
        return (
          ...
          >
            ...
            <span className="touch:text-xs text-muted-foreground ml-auto text-[10px] tracking-wide">
              {pillLabel}
            </span>
          </div>
        );

...
import { CompareModelsRail } from '@/components/marketing/compare-models/compare-models-rail';
...
import type { CompareModelsPairPageProps } from '@/types/compare-models-component.types';
...
import {
  buildCompareModelsRelatedLinks,
  buildCompareModelsSiblings,
  getCompareModelsContent,
  getCompareModelsPairContent,
...
export async function CompareModelsPairPage({
  pair,
}: CompareModelsPairPageProps): Promise<React.ReactElement> {
  ...
  const { labels, hub } = getCompareModelsContent(locale);
  const content = getCompareModelsPairContent(locale, pair);
  const registryEntry = getPageBySlugAndLocale(getModelFamilyPairSlug(pair), locale);
  const hubEntry = getPageBySlugAndLocale(COMPARE_MODELS_HUB_SLUG, locale);

  const siteUrl = getSiteUrl();
  const canonicalPath = localisePath(getModelFamilyPairPath(pair), locale);
  const canonicalUrl = new URL(canonicalPath, siteUrl).toString();
  const hubPath = localisePath(COMPARE_MODELS_HUB_PATH, locale);
        ...
        <div className="editorial-comparison">
          ...
          <nav className="editorial-comparison__rail" aria-label={labels.relatedTitle}>
            ...
            <ul className="editorial-comparison__rail-list">
              {buildCompareModelsRelatedLinks(locale, pair).map((link) => (
                <li key={link.path}>
          ...
          <CompareModelsRail
            ...
            items={buildCompareModelsSiblings(locale, pair)}
          />
        </div>
      </EditorialPageShell>
    </>

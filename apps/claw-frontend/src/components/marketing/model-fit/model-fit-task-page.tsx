...
import { ModelFitRail } from '@/components/marketing/model-fit/model-fit-rail';
...
import type { ModelFitTaskPageProps } from '@/types/model-fit-component.types';
...
import {
  buildModelFitRelatedLinks,
  buildModelFitSiblings,
  getModelFitContent,
  getModelFitTaskContent,
} from '@/utilities/model-fit.utility';
import { buildLearnTopicJsonLd, serializeJsonLd } from '@/utilities/structured-data.utility';

export async function ModelFitTaskPage({
  task,
}: ModelFitTaskPageProps): Promise<React.ReactElement> {
  ...
  const { labels, hub } = getModelFitContent(locale);
  const content = getModelFitTaskContent(locale, task);
  const registryEntry = getPageBySlugAndLocale(getModelFitTaskSlug(task), locale);
  const hubEntry = getPageBySlugAndLocale(MODEL_FIT_HUB_SLUG, locale);

  const siteUrl = getSiteUrl();
  const canonicalPath = localisePath(getModelFitTaskPath(task), locale);
  const canonicalUrl = new URL(canonicalPath, siteUrl).toString();
  const hubPath = localisePath(MODEL_FIT_HUB_PATH, locale);
        ...
        <div className="editorial-comparison">
          ...
          <nav className="editorial-comparison__rail" aria-label={labels.relatedTitle}>
            ...
            <ul className="editorial-comparison__rail-list">
              {buildModelFitRelatedLinks(locale, task).map((link) => (
                <li key={link.path}>
          ...
          <ModelFitRail label={labels.backToHub} items={buildModelFitSiblings(locale, task)} />
        </div>
      </EditorialPageShell>
    </>
  );

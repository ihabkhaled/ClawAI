...
import { UseCaseRail } from '@/components/marketing/use-cases/use-case-rail';
...
import type { UseCaseTaskPageProps } from '@/types/use-cases-component.types';
...
import {
  buildUseCaseRelatedLinks,
  buildUseCaseSiblings,
  getUseCaseTaskContent,
  getUseCasesClusterContent,
} from '@/utilities/use-cases-cluster.utility';

export async function UseCaseTaskPage({ task }: UseCaseTaskPageProps): Promise<React.ReactElement> {
  ...
  const { labels } = getUseCasesClusterContent(locale);
  const content = getUseCaseTaskContent(locale, task);
  const registryEntry = getPageBySlugAndLocale(getUseCaseTaskSlug(task), locale);
  const hubEntry = getPageBySlugAndLocale(USE_CASES_HUB_SLUG, locale);

  const siteUrl = getSiteUrl();
  const canonicalPath = localisePath(getUseCaseTaskPath(task), locale);
  const canonicalUrl = new URL(canonicalPath, siteUrl).toString();
  const hubPath = localisePath(USE_CASES_HUB_PATH, locale);
      ...
      >
        <div className="editorial-comparison">
          <p className="editorial-comparison__stamp">
            ...
            {' · '}
            {labels.lastReviewed}:{' '}
            <time dateTime={USE_CASES_REVIEW_DATE}>{USE_CASES_REVIEW_DATE}</time>
          </p>
          ...
          <nav className="editorial-comparison__rail" aria-label={labels.relatedTitle}>
            ...
            <ul className="editorial-comparison__rail-list">
              {buildUseCaseRelatedLinks(locale, task).map((link) => (
                <li key={link.path}>
          ...
          <UseCaseRail label={labels.backToHub} items={buildUseCaseSiblings(locale, task)} />
        </div>
      </EditorialPageShell>
    </>
  );

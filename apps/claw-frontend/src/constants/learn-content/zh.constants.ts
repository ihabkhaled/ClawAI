  ...
  hub: {
    ...
    cardSummaries: {
      ...
      [LearnTopic.HOW_AI_TOOL_CALLING_WORKS]:
        '模型从来不会执行任何操作——它只是提出一次调用，由你的应用决定接下来发生什么。',
      [LearnTopic.WHAT_ARE_STRUCTURED_AI_OUTPUTS]:
        '向模型要求 JSON 只是一个请求；只有部分机制才能真正保证它符合你的结构定义。',
      [LearnTopic.WHY_AI_HALLUCINATES]:
        '为什么模型会用和答对时同样的自信说出错误答案——以及真正能降低这一点的方法。',
      ...
      [LearnTopic.HOW_TO_EVALUATE_AI_MODELS]:
        '把工作交给一个模型之前，真正该测试的是什么——而不是排行榜上的一个数字。',
      [LearnTopic.HOW_TO_READ_AI_BENCHMARKS]:
        '一个跑分数字真正衡量的是什么，以及它在你开始测试之前就可能误导你的地方。',
      [LearnTopic.WHAT_IS_PROMPT_INJECTION]:
        '不是你写的文本仍然可以给模型下指令——这意味着什么，以及为什么更聪明的模型也无法完全解决它。',
      [LearnTopic.AI_DATA_PRIVACY_AND_RETENTION]:
        '你在 AI 产品里输入的内容究竟会发生什么——谁能看到、是否会用来训练模型，以及会保留多久。',
    },
  },
  topics: {
    [LearnTopic.HOW_LANGUAGE_MODELS_GENERATE_ANSWERS]: {
    ...
    [LearnTopic.HOW_AI_TOOL_CALLING_WORKS]: {
      seo: {
        title: 'AI 工具调用到底是怎么运作的？',
        description:
          '调用工具的模型自己从来不会真正执行任何操作——它只是提出一个名称和一组参数，由你的应用决定是否执行这次调用。请求-响应循环具体如何运作，以及为什么这个提议只是一种猜测，而不是保证。',
        keywords: ['工具调用如何运作', 'LLM function calling 解析', 'AI 工具使用机制'],
      },
      eyebrow: '基础原理',
      title: 'AI 工具调用到底是怎么运作的？',
      summary:
        '工具调用，有时也叫 function calling，让模型可以请求代它完成某件事：查询数据库、调用某个 API、执行一次计算。真正让人意外的是，模型在那一刻实际做了什么——它什么都没有执行。它输出的是一个结构化请求，指明一个工具的名称和参数，至于是否要据此采取行动、怎么行动，都由你的应用决定。',
      sections: [
        {
          id: 'what-tool-calling-actually-is',
          heading: '给模型的是一份菜单，不是一个键盘',
          paragraphs: [
    ...
    [LearnTopic.WHAT_ARE_STRUCTURED_AI_OUTPUTS]: {
      seo: {
        ...
        keywords: ['结构化输出是什么', 'LLM JSON 模式解析', '按结构定义约束的 AI 生成'],
      },
      eyebrow: '基础原理',
      title: 'AI 的结构化输出是什么？',
      summary:
        '结构化输出是指模型的回答被塑造成符合某种特定格式——通常是符合一份定义好的结构的 JSON——而不是自由文本，这样代码就能不靠猜测直接解析它。真正让人意外的是，"要求模型返回 JSON"和"保证模型返回有效的 JSON"是两种不同的说法，而真正能兑现后者的只是部分机制。',
      sections: [
        {
          id: 'what-structured-output-means',
          heading: '结构化意味着下游代码可以依赖这份形状',
          paragraphs: [
    ...
    [LearnTopic.WHY_AI_HALLUCINATES]: {
      seo: {
        ...
        keywords: ['AI 为什么会产生幻觉', '语言模型幻觉原理解析', 'AI 编造信息'],
      },
      eyebrow: '基础概念',
      title: 'AI 为什么会产生幻觉？',
      summary:
        '幻觉指模型把错误的内容当作事实说出来，没有任何保留，也没有任何暗示它在猜测。这是因为语言模型被训练来生成统计上最可能的下一个词元，而不是去核对某个说法是否属实——一句流畅、自信又错误的话，和一句流畅、自信又正确的话，出自完全相同的生成过程。',
      sections: [
        {
          id: 'a-confident-wrong-answer-not-a-crash',
          heading: '这是一个自信的错误答案，不是模型能标记出来的故障',
          paragraphs: [
    ...
    [LearnTopic.HOW_TO_EVALUATE_AI_MODELS]: {
      seo: {
        title: '如何为自己的用途评估 AI 模型',
        description:
          '排行榜上的数字告诉你一个模型在别人任务上的表现，而不是在你自己任务上的表现。真正能预测它能否胜任你的工作的，是权衡质量、成本、延迟和隐私之间的取舍——这些维度是单一数字永远无法体现出来的。',
        keywords: ['如何评估 AI 模型', '选择 AI 模型', '模型比较标准'],
      },
      eyebrow: '基础概念',
      title: '如何为自己的用途评估 AI 模型',
      summary:
        '为自己的工作评估一个模型，意味着用自己的任务去测试它，而不是相信一个在别人任务上算出来的分数。一个在公开排行榜上名列前茅的模型，一旦把质量拿来对照成本、延迟，以及你到底被允许把什么发给它这些因素权衡后，仍可能是某项具体工作的错误选择。',
      sections: [
        {
        ...
        },
        {
          ...
          heading: '每个模型都有已知的薄弱环节——在依赖它之前先找到你的那个',
          paragraphs: [
    ...
    [LearnTopic.HOW_TO_READ_AI_BENCHMARKS]: {
      seo: {
        title: '如何阅读 AI 跑分而不被误导',
        description:
          '跑分数字看起来很精确，这正是让人容易过度信任它的原因。像 MMLU 或 HumanEval 这样的分数究竟衡量的是什么、跑分数字常见的误导方式，以及在把某个分数当作对你的用例有意义之前应该检查什么。',
        keywords: ['如何阅读 AI 跑分', 'AI 跑分数字解析', '理解语言模型跑分'],
      },
      eyebrow: '基础概念',
      title: '如何阅读 AI 跑分而不被误导',
      summary:
        '一个跑分数字衡量的是模型在一组固定、具体的测试题上的表现——仅此而已，不多也不少。这个数字看起来精确又客观，而这恰恰是让人容易过度信任它的原因：单独一个数字无法告诉你测试覆盖了什么、模型是被怎样提问的，也无法告诉你这些题目是否在模型真正接触到它们之前，就已经泄露进了训练数据。',
      sections: [
        {
          id: 'a-benchmark-is-one-fixed-test-not-a-general-measure',
          heading: '跑分是一次固定的测试，不是通用的能力衡量标准',
          paragraphs: [
        ...
        },
        {
          ...
          heading: '跑分会饱和——一旦大多数模型都能通过，它就不再有用了',
          paragraphs: [
    ...
    [LearnTopic.WHAT_IS_PROMPT_INJECTION]: {
      seo: {
        ...
        keywords: ['什么是提示词注入', '提示词注入攻击解析', '间接提示词注入'],
      },
      eyebrow: '基础概念',
      title: '什么是提示词注入？',
      summary:
        '提示词注入指的是并非来自你、却仍然能给模型下达指令的文本——藏在它读取的网页里、它要总结的文档里，或它调用的工具结果里。语言模型没有一种可靠的内置方式来区分"用户的指令"和"碰巧看起来像指令的文本"，因为两者以完全相同的形式抵达：上下文窗口里的文字。',
      sections: [
        {
        ...
        },
        {
          ...
          heading: '过滤和限权能降低风险；但两者都无法彻底消除风险',
          paragraphs: [
        ...
        },
        {
          ...
          heading: '模型读取的内容是不可信的输入，不是中立的事实来源',
          paragraphs: [
    ...
    [LearnTopic.AI_DATA_PRIVACY_AND_RETENTION]: {
      seo: {
        ...
        keywords: ['AI 数据隐私解析', 'AI 数据保留期', '我的数据是否被用来训练 AI'],
      },
      eyebrow: '基础概念',
      title: 'AI 数据隐私与保留期解析',
      summary:
        '你在 AI 产品里输入的内容会经历什么，取决于几个具体的问题：它究竟有没有离开你自己的基础设施、提供商是否用它来训练未来的模型、它会被存储多久，以及还有谁——分包处理方、日志系统、人工审核人员——可能在途中看到它。这些都没有统一答案，因产品和提供商而异，这也正是为什么弄清楚该问什么问题很重要。',
      sections: [
        {
          id: 'where-your-input-actually-goes',
          heading: '第一个问题：它究竟有没有离开你的基础设施',
          paragraphs: [
        ...
        },
        {
          ...
          heading: '你的数据是否用于训练未来的模型，与是否被存储是两个独立的问题',
          paragraphs: [
        ...
        },
        {
          ...
          heading: '你数据的流转路径往往不止涉及 AI 提供商一方',
          paragraphs: [
        ...
        },
        {
          ...
          heading: '在任何 AI 工具里粘贴敏感内容之前，真正该检查什么',
          paragraphs: [

...
export default function LocalModelsPage(): React.ReactElement {
  ...
  const columns: DataTableColumn<LocalModel>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Badge variant="outline">
          {RUNTIME_TYPE_LABELS[row.runtime] ?? row.runtime}
        </Badge>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Family",
      render: (row) => (
        <span className="text-muted-foreground">{row.family ?? "N/A"}</span>
      ),
    },
    {
      key: "parameters",
    ...
    },
    {
      ...
      render: (row) => (
        <Select
          onValueChange={(role) => handleAssignRole(row.id, role)}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current status of the local model runtime
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Download a new model from the registry
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-end gap-3">
              ...
              <div className="w-[160px] space-y-2">
                <label
                  htmlFor="pull-model-runtime"
                  className="text-sm font-medium"
                >
                  Runtime
                </label>
                <Select value={pullRuntime} onValueChange={setPullRuntime}>
      ...
      <div className="mt-6">
        ...
        {isLoading ? (
          <LoadingSpinner label="Loading local models..." />
        ) : models.length === 0 ? (
          <EmptyState
            icon={HardDrive}

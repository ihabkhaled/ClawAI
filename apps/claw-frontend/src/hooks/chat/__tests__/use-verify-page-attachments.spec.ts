...
import type { SendVerifyPayload } from '@/types';
...
vi.mock('@/repositories/chat/chat.repository', () => ({
  chatRepository: {
    sendVerify: (data: SendVerifyPayload) => sendMock(data),
  },
}));
...
vi.mock('@/hooks/chat/use-verify-poll', () => ({
  useVerifyPoll: () => ({
    verifyResult: null,
    isPolling: false,
    isVerifyReady: false,
...
describe('useVerifyPage — fileIds round-trip', () => {
  beforeEach(() => {
    sendMock.mockReset();
  ...
  it('omits fileIds from the payload when nothing is attached', async () => {
    const { result } = renderHook(() => useVerifyPage(), { wrapper });
    ...
    const payload = sendMock.mock.calls[0]?.[0] as SendVerifyPayload;

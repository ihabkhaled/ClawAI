...
import type { SendPipelinePayload } from '@/types';
...
vi.mock('@/repositories/chat/chat.repository', () => ({
  chatRepository: {
    sendPipeline: (data: SendPipelinePayload) => sendMock(data),
  },
}));
...
vi.mock('@/hooks/chat/use-pipeline-poll', () => ({
  usePipelinePoll: () => ({
    pipelineResult: null,
    isPolling: false,
...
vi.mock('@/hooks/chat/use-chat-stream', () => ({
  useChatStream: () => ({ progressStages: [], streamError: null }),
}));
...
describe('usePipelinePage — fileIds round-trip', () => {
  beforeEach(() => {
    sendMock.mockReset();
  ...
  it('omits fileIds from the payload when nothing is attached', async () => {
    const { result } = renderHook(() => usePipelinePage(), { wrapper });
    ...
    const payload = sendMock.mock.calls[0]?.[0] as SendPipelinePayload;

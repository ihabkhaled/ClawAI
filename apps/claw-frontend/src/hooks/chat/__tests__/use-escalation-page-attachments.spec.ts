...
import type { EscalationChainRequest } from '@/types';
...
vi.mock('@/repositories/chat/chat.repository', () => ({
  chatRepository: {
    sendEscalationChain: (data: EscalationChainRequest) => sendMock(data),
  },
}));
...
describe('useEscalationPage — fileIds round-trip', () => {
  beforeEach(() => {
    sendMock.mockReset();

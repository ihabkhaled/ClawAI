...
import type { SendCostEnsemblePayload } from '@/types';
...
vi.mock('@/repositories/chat/chat.repository', () => ({
  chatRepository: {
    sendCostEnsemble: (data: SendCostEnsemblePayload) => sendMock(data),
  },
}));
...
vi.mock('@/hooks/chat/use-cost-ensemble-poll', () => ({
  useCostEnsemblePoll: () => ({
    costEnsembleResult: null,
    isPolling: false,
    isCostEnsembleReady: false,
    isCostEnsembleError: false,
...
describe('useCostEnsemblePage — fileIds round-trip', () => {
  beforeEach(() => {
    sendMock.mockReset();
  ...
  it('omits fileIds from the payload when nothing is attached', async () => {
    const { result } = renderHook(() => useCostEnsemblePage(), { wrapper });
    ...
    const payload = sendMock.mock.calls[0]?.[0] as SendCostEnsemblePayload;

...
import type { SendRolePackPayload } from '@/types';
...
vi.mock('@/repositories/chat/chat.repository', () => ({
  chatRepository: {
    sendRolePack: (data: SendRolePackPayload) => sendMock(data),
  },
}));
...
vi.mock('@/hooks/chat/use-role-pack-poll', () => ({
  useRolePackPoll: () => ({
    rolePackResult: null,
    isPolling: false,
    isRolePackReady: false,
    isRolePackError: false,
    handleViewInThread: () => undefined,
  }),
}));

vi.mock('@/hooks/chat/use-role-pack-stages', () => ({
  useRolePackStages: () => ({ stages: [] }),
}));
...
describe('useRolePackPage — fileIds round-trip', () => {
  beforeEach(() => {
    sendMock.mockReset();
  ...
  it('omits fileIds from the payload when nothing is attached', async () => {
    const { result } = renderHook(() => useRolePackPage(), { wrapper });
    ...
    const payload = sendMock.mock.calls[0]?.[0] as SendRolePackPayload;

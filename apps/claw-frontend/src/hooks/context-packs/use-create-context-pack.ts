...
export function useCreateContextPack() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      logger.info({
        component: 'memory',
        action: 'create-context-pack-success',
    ...
    onError: (error: Error) => {
      logger.error({
        component: 'memory',
        action: 'create-context-pack-error',
        message: error.message,
      });
      showToast.apiError(error, resolveApiErrorMessage(error, t, t('context.packCreateFailed')));
    },
  });

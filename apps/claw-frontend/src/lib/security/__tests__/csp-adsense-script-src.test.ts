...
describe('CSP script-src for AdSense', () => {
  it('names the loader host in development, where strict-dynamic is absent', () => {
    ...
    const csp = buildContentSecurityPolicy({ nonce: 'n', isDev: true, adsenseEnabled: true });
  ...
  it('omits the loader host in development when AdSense is off', () => {
    const csp = buildContentSecurityPolicy({ nonce: 'n', isDev: true, adsenseEnabled: false });
  ...
  it('does not weaken production script-src with a host allowlist', () => {
    ...
    // XSS-surface change.
    const csp = buildContentSecurityPolicy({ nonce: 'n1', isDev: false, adsenseEnabled: true });
  ...
  it('keeps frame, img and connect hosts gated on AdSense being enabled', () => {
    const on = buildContentSecurityPolicy({ nonce: 'n', isDev: false, adsenseEnabled: true });
    const off = buildContentSecurityPolicy({ nonce: 'n', isDev: false, adsenseEnabled: false });

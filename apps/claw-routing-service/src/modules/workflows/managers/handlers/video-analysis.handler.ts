...
@Injectable()
export class VideoAnalysisHandler implements IWorkflowHandler {
  readonly kind = WorkflowKind.VIDEO_ANALYSIS;
  canHandle(_c: WorkflowExecutionContext): boolean {
    throw new Error('SCAFFOLD-R3 — VideoAnalysisHandler.canHandle not implemented');
  }
  plan(_c: WorkflowExecutionContext): WorkflowPlan {
    throw new Error('SCAFFOLD-R3 — VideoAnalysisHandler.plan not implemented');
  }

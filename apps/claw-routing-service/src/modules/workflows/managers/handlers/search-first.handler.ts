...
@Injectable()
export class SearchFirstHandler implements IWorkflowHandler {
  readonly kind = WorkflowKind.SEARCH_FIRST;
  canHandle(_c: WorkflowExecutionContext): boolean {
    throw new Error('SCAFFOLD-R3 — SearchFirstHandler.canHandle not implemented');
  }
  plan(_c: WorkflowExecutionContext): WorkflowPlan {
    throw new Error('SCAFFOLD-R3 — SearchFirstHandler.plan not implemented');
  }

...
@Injectable()
export class ImageAnalysisHandler implements IWorkflowHandler {
  readonly kind = WorkflowKind.IMAGE_ANALYSIS;
  canHandle(_c: WorkflowExecutionContext): boolean {
    throw new Error('SCAFFOLD-R3 — ImageAnalysisHandler.canHandle not implemented');
  }
  plan(_c: WorkflowExecutionContext): WorkflowPlan {
    throw new Error('SCAFFOLD-R3 — ImageAnalysisHandler.plan not implemented');
  }

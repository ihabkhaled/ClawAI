...
@Injectable()
export class AudioTranscribeHandler implements IWorkflowHandler {
  readonly kind = WorkflowKind.AUDIO_TRANSCRIBE;
  canHandle(_c: WorkflowExecutionContext): boolean {
    throw new Error('SCAFFOLD-R3 — AudioTranscribeHandler.canHandle not implemented');
  }
  plan(_c: WorkflowExecutionContext): WorkflowPlan {
    throw new Error('SCAFFOLD-R3 — AudioTranscribeHandler.plan not implemented');
  }

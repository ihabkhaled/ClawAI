  ...
  private readonly logger = new Logger(GlobalExceptionFilter.name);
    ...
    if (exception instanceof BusinessException) {
      ...
      if (typeof exResponse === 'object' && exResponse !== null) {
        ...
        message = (responseObj['messageKey'] as string | undefined) ?? message;
      ...
      if (typeof exResponse === 'object' && exResponse !== null) {
        const responseObj = exResponse as Record<string, unknown>;
        message = (responseObj['message'] as string | undefined) ?? exception.message;
        errors = responseObj['errors'] as unknown[] | undefined;
      } else {
        message = exception.message;
      }
    } else if (exception instanceof Error) {
      // The stack goes to the log sink, never to the client: an unexpected
      // failure in a payment path must not leak provider or query internals.

...
export class FleetController {
  constructor(
    private readonly organizations: OrganizationMembershipService,
  ...
  // V2 Stream 07 — device matrix for fleet governance: every member's
  // devices, last-seen + pending capability count. OWNER/ADMIN only.
  @Get(':id/devices')
  async listDevices(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
  ): Promise<DeviceMatrixRow[]> {
    return this.organizations.listDevices(id, user.id);
  }
  ...
  async addMember(
    ...
    return this.organizations.addMember(id, user.id, dto);
  }
}

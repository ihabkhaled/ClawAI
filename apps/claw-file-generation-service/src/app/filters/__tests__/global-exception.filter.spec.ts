...
describe('GlobalExceptionFilter', () => {
  ...
  // An exported answer over Express's default body limit used to come back as
  // a 500, so the user saw "server error" for a request that was simply too big.
  it.each([
  ...
  it('still maps an HttpException to its own status', () => {
    const sent = send(new NotFoundException('File generation not found'));

    expect(sent.status).toBe(HttpStatus.NOT_FOUND);
    expect(sent.body).toMatchObject({ message: 'File generation not found' });
  });
});

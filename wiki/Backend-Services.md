> **Wiki source:** [`context/service-catalog.md`](https://github.com/ihabkhaled/ClawAI/blob/main/context/service-catalog.md) on the current `main` branch.
...
- **Consumes:** almost every lifecycle event (agent capability, ai_action, connector, file, llamacpp, memory, message.completed, routing, user, workspace sync/action). See [event-flow-map.md](https://github.com/ihabkhaled/ClawAI/blob/main/context/event-flow-map.md).
...
- **Pitfalls:** **no `*_SERVICE_PORT` constant** — port is `CLIENT_LOGS_PORT` env-only. Flagged in the inventory audit. See [port-and-service-map.md](https://github.com/ihabkhaled/ClawAI/blob/main/context/port-and-service-map.md).

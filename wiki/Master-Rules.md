> **Wiki source:** [`rules/00-master-rules.md`](https://github.com/ihabkhaled/ClawAI/blob/main/rules/00-master-rules.md) on the current `main` branch.
...
**`--no-verify` is banned** ([ADR-061](https://github.com/ihabkhaled/ClawAI/blob/main/docs/13-adr/adr-061-git-hook-policy-no-bypass.md)). The hooks now run the _affected_ lane only, so they are scoped and fast — a hook failure is a real problem in something you staged, and skipping it just moves the failure to CI. The only exception is a documented incident procedure with explicit human sign-off (`docs/exceptions/README.md`).
...
[`26-prompt-pack-intake-protocol.md`](https://github.com/ihabkhaled/ClawAI/blob/main/rules/26-prompt-pack-intake-protocol.md) runs first,
in full, regardless of how small or urgent the pack looks. Runbook:
[`../skills/execute-prompt-pack.md`](https://github.com/ihabkhaled/ClawAI/blob/main/skills/execute-prompt-pack.md). Summary:

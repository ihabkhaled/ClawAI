> **Wiki source:** [`docs/08-runtime-devops/nginx-reference.md`](https://github.com/ihabkhaled/ClawAI/blob/main/docs/08-runtime-devops/nginx-reference.md) on the current `main` branch.
...
**Host ports**: 443 (HTTPS), 4000 → 80, and in prod also 80 → 80

The configuration is split across three files, all bind-mounted read-only:

| File                            | Mounted at                       | Holds                                                                       |
| ------------------------------- | -------------------------------- | --------------------------------------------------------------------------- |
| `infra/nginx/nginx.conf`        | `/etc/nginx/nginx.conf`          | `http{}` settings, the port-80 block, the default (mkcert) TLS server block |
| `infra/nginx/locations.conf`    | `/etc/nginx/claw/locations.conf` | **every** `location` — the routing table below                              |
| `infra/nginx/public-tls/*.conf` | `/etc/nginx/claw/public-tls/`    | generated Let's Encrypt server blocks (gitignored; empty on a laptop)       |

The routing table is a separate file because a production server serves the same
routes under two certificates — mkcert internally, Let's Encrypt for the public
domain — and each TLS server block `include`s it. Duplicating ~52 `location`
blocks per certificate guarantees the copies drift, and a drifted route is an
outage that reproduces on only one hostname. See
[tls-setup.md](https://github.com/ihabkhaled/ClawAI/blob/main/docs/08-runtime-devops/tls-setup.md#public-tls-on-a-real-domain).
...
1. Edit the right file:
   - **adding or changing a route** → `infra/nginx/locations.conf`. It is
     included by every TLS server block, so the route is served under both the
     mkcert and the Let's Encrypt certificate automatically. Never copy a
     `location` into a server block.
   - **TLS, timeouts, rate-limit zones, security headers** → `infra/nginx/nginx.conf`.
   - **never** hand-edit `infra/nginx/public-tls/*.conf`; it is generated by
     `scripts/install-letsencrypt.sh` and rewritten on every run.
2. Check the config parses before applying it — a bad reload on a running
   server is avoidable:
   ```bash
   docker exec claw-nginx nginx -t
   ```
3. Apply it. A reload is graceful and drops no connections:
   ```bash
   docker exec claw-nginx nginx -s reload
   ```
   A **restart** is only needed when the mount set itself changed (a new file
   or a new published port), and that requires a recreate, not a restart:
...
All three files are mounted read-only (`:ro`), so edits on the host take effect
on the next reload — but a file that was not mounted at container-create time
will not appear by reloading.

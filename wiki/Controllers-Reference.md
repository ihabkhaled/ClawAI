> **Wiki source:** [`docs/04-backend/controllers-reference.md`](https://github.com/ihabkhaled/ClawAI/blob/main/docs/04-backend/controllers-reference.md) on the current `main` branch.
...
| `GET`   | `/chat-messages/thread/:threadId` | Authenticated | List messages in a thread (cursor-paginated)     | `ListMessagesQueryDto` (`listMessagesQuerySchema`) -- before (cursor), limit                 | `CursorPaginatedResult<ChatMessage>` | `THREAD_NOT_FOUND`, `THREAD_ACCESS_DENIED`                   |

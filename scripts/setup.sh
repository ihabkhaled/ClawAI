...
echo ""
echo "  AI mode: API-only by default (no local model downloads). Add a provider"
echo "           (OpenAI, Gemini, Anthropic, ...) in the Connectors UI after login."
echo "           For the full local runtime (Ollama, ComfyUI, Stable Diffusion),"
echo "           set CLAW_LOCAL_AI=true in .env or run ./scripts/claw.sh --local-ai up"
echo ""
